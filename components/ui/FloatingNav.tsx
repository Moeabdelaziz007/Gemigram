...
import { ChevronRight, Globe, Home, LayoutDashboard, LogOut, Plus, Settings, User, Users } from 'lucide-react';
...
const ORBS_CONFIG = [
  { id: 'home', path: '/dashboard', icon: <Home aria-hidden="true" className="w-5 h-5" />, label: BRAND.labels.nav.home },
  { id: 'galaxy', path: '/galaxy', icon: <Globe aria-hidden="true" className="w-5 h-5" />, label: BRAND.labels.nav.galaxy },
  { id: 'hub', path: '/hub', icon: <Users aria-hidden="true" className="w-5 h-5" />, label: BRAND.labels.nav.hub },
  { id: 'forge', path: '/forge', icon: <Plus aria-hidden="true" className="w-5 h-5" />, label: BRAND.labels.nav.forge },
  { id: 'workspace', path: '/workspace', icon: <LayoutDashboard aria-hidden="true" className="w-5 h-5" />, label: BRAND.labels.nav.workspace },
  { id: 'marketplace', path: '/marketplace', icon: <Globe aria-hidden="true" className="w-5 h-5" />, label: BRAND.labels.nav.marketplace },
  { id: 'settings', path: '/settings', icon: <Settings aria-hidden="true" className="w-5 h-5" />, label: BRAND.labels.nav.settings },
  { id: 'about', path: '/about', icon: <ChevronRight aria-hidden="true" className="w-5 h-5" />, label: BRAND.labels.nav.about },
];
...
export function FloatingNav({ currentView, user, onLogin, onLogout }: FloatingNavProps) {
  ...
  return (
    <>
      ...
      <motion.aside initial={{ x: -80, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="hidden md:flex flex-col items-center py-8 w-20 xl:w-64 glass-strong border-r border-gemigram-neon/[0.06] z-[90] shrink-0">
        <div className="mb-12">
        ...
        <div className="mt-auto px-3 w-full space-y-4">
          {user ? (
            <div className="flex flex-col gap-2">
              ...
              <button onClick={onLogout} title="Terminate session" aria-label="Terminate session" className="flex items-center gap-4 px-4 py-3 rounded-2xl text-red-400 hover:bg-red-400/10 transition-colors border border-transparent hover:border-red-400/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gemigram-neon focus-visible:ring-offset-2 focus-visible:ring-offset-black/80">
            ...
            <button onClick={onLogin} className="w-full py-4 rounded-2xl bg-gemigram-neon text-black font-black uppercase text-[10px] tracking-widest shadow-[0_0_30px_rgba(16,255,135,0.3)] hover:scale-[1.02] transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gemigram-neon focus-visible:ring-offset-2 focus-visible:ring-offset-black/80">
              Access_System
            </button>
          )}
        </div>
      ...
      <motion.nav initial={{ y: 80 }} animate={{ y: 0 }} className="md:hidden fixed bottom-6 left-6 right-6 h-16 aether-glass border border-white/10 rounded-2xl z-[100] flex items-center justify-around px-2">
