...
export default function AppShell({ children }: AppShellProps) {
  ...
  return (
    <div className="flex h-[100dvh] w-full overflow-hidden bg-theme-primary font-sans text-white selection:bg-gemigram-neon/30" data-visual-tier={tier}>
      <FloatingNav currentView={currentView} user={user} onLogin={login} onLogout={logout} />

      <div className="relative flex min-w-0 flex-1 flex-col">
        <header className={`z-[80] flex h-14 shrink-0 items-center justify-between border-b border-gemigram-neon/[0.06] px-4 md:h-16 md:px-8 ${shellGlassClass}`}>
          <div className="flex items-center gap-4 md:gap-6">
            <div className="flex items-center gap-3">
              <div className="flex h-8 w-8 items-center justify-center rounded-lg border border-gemigram-neon/30 bg-gemigram-neon/10">
                <AetherLogo size={14} />
              </div>
              <div className="flex flex-col">
                <span className="text-[10px] font-black uppercase leading-none tracking-[0.2em] text-gemigram-neon">{BRAND.product.name}</span>
                <span className="text-[8px] font-mono uppercase tracking-[0.1em] text-white/30">{BRAND.product.systemVersion}</span>
              </div>
            </div>
            ...
            <div className="hidden items-center gap-6 lg:flex">
              ...
              <div className="group flex cursor-pointer items-center gap-2 border-l border-white/5 pl-6">
          ...
          <div className="hidden flex-1 flex-col items-center text-center xl:flex">
          ...
          <div className="flex items-center gap-4 md:gap-8">
            <div className="hidden items-center gap-4 border-r border-white/5 pr-6 md:flex">
            ...
            <div className="flex items-center gap-4">
              <div className="flex flex-col items-end">
                ...
                <span className="leading-none text-[8px] font-mono font-bold uppercase tracking-widest text-white/20">
              ...
              <div className="group relative">
                <motion.div
                  whileHover={allowAmbientMotion ? { scale: 1.03 } : undefined}
                ...
                <div className="pointer-events-none absolute right-0 top-full mt-4 translate-y-2 opacity-0 transition-all duration-300 group-hover:pointer-events-auto group-hover:translate-y-0 group-hover:opacity-100">
                  <div className={`w-64 rounded-2xl border border-white/10 p-6 shadow-2xl ${isMobile ? 'glass-subtle' : 'glass-medium'}`}>
                    <div className="mb-6 flex items-center gap-4">
                      ...
                      <div className="flex flex-col overflow-hidden">
                        <span className="max-w-full truncate text-xs font-black text-white">{user?.displayName || 'Anonymous Architect'}</span>
                        <span className="truncate text-[9px] font-mono uppercase text-gemigram-neon/60">{user?.email || 'OFFLINE'} · {linkType || 'LOCAL'}</span>
                      </div>
                    </div>
        ...
        <main className="relative z-10 flex-1 overflow-hidden">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="custom-scrollbar h-full w-full overflow-x-hidden overflow-y-auto pb-10"
            >
              {children}
            </motion.div>
          </AnimatePresence>
        ...
        <footer className="pointer-events-none absolute bottom-6 left-1/2 z-50 hidden -translate-x-1/2 xl:flex">
          <div className="pointer-events-auto flex items-center gap-5 rounded-full border border-gemigram-neon/[0.08] px-6 py-2.5 glass-subtle">
